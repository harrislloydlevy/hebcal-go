/*
 * test_anniversary.c - 1:1 port of anniversary_test.go to C.
...
/* Parse "YYYY-MM-DD" and create an HDate from the Gregorian date. */
static heb_hdate_t hd_from_iso(const char *iso) {
    int y, m, d;
    sscanf(iso, "%d-%d-%d", &y, &m, &d);
    return heb_hdate_from_gregorian(y, (heb_month_greg_t)m, d);
}

/* Format a Gregorian date as M/D/YYYY (matching Go's fmt.Sprintf("%d/%d/%d")). */
...
HEBTEST_FUNC(test_anniversary) {
    /* TestYahrzeit */
    {
        struct { const char *date; const char *name; const char *expected; } items[] = {
            {"2017-01-13", "General",
        ...
        for (size_t i = 0; i < sizeof(items)/sizeof(items[0]); i++) {
            heb_hdate_t hd = hd_from_iso(items[i].date);
            /* Split expected by spaces and check each year */
            char expected[2048];
            strncpy(expected, items[i].expected, sizeof(expected) - 1);
            expected[sizeof(expected) - 1] = '\0';

            char *tok = strtok(expected, " ");
            for (int j = 0; j < 25 && tok != NULL; j++) {
                int hyear = j + 5778;
                heb_hdate_t result;
                int rc = heb_anniversary_yahrzeit(hyear, &hd, &result);
                HEB_CHECK_INT_EQ(rc, 0);
                heb_gdate_t gd = heb_hdate_greg(&result);
                char actual[32];
                greg_mdy(&gd, actual, sizeof(actual));
                HEB_CHECK_STR_EQ(actual, tok);
                tok = strtok(NULL, " ");
            }
        }
    }
    ...
    /* TestBirthday */
    {
        ...
        for (size_t i = 0; i < sizeof(items)/sizeof(items[0]); i++) {
            ...
            char *tok = strtok(expected, " ");
            for (int j = 0; j < 21 && tok != NULL; j++) {
                int hyear = j + 5753;
                heb_hdate_t result;
                int rc = heb_anniversary_birthday(hyear, &hd, &result);
                HEB_CHECK_INT_EQ(rc, 0);
                heb_gdate_t gd = heb_hdate_greg(&result);
                char actual[32];
                greg_mdy(&gd, actual, sizeof(actual));
    ...
    /* TestBirthdayOnOrBefore */
    {
        heb_hdate_t orig;
        heb_hdate_create(&orig, 5753, HEB_H_ADAR1, 9);

        heb_hdate_t birthday;
        int rc = heb_anniversary_birthday(5753, &orig, &birthday);
        HEB_CHECK_INT_EQ(rc, 0);
        HEB_CHECK_INT_EQ(birthday.year, orig.year);
        HEB_CHECK_INT_EQ(birthday.month, orig.month);
        HEB_CHECK_INT_EQ(birthday.day, orig.day);

        /* Year before original should fail */
        rc = heb_anniversary_birthday(5752, &orig, &birthday);
        HEB_CHECK_INT_EQ(rc, 1);
    }
